...
const cliOptions = [...ProviderOptions, ...ApiOptions, ...EntrypointOptions, ...CliOther];
const snippets = [...dockerHttpSnippets, ...dockerTcpSnippets, ...traefikEnvars];
const completers = [...traefikCompleter, ...dockerHttpCompleter, ...dockerTcpCompleter];

export async function activate(context: vscode.ExtensionContext) {
  context.subscriptions.push(...cliOptions, ...snippets, ...completers);

  // @todo This seems to be only for use with the redhat yaml extension? See the
  // getYamlApi implementation. It anoyingly yells at us if that extension is
  // not present
